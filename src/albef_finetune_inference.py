import torch
from torch.utils.data import SequentialSampler, DataLoader

from albef_finetune_config import parse_args
from finetune_data_helper import MultiModalDataset
from category_id_map import lv2id_to_category_id
from albef_finetune_model import ALBEF

import numpy as np
import os

def inference():
    args = parse_args()
    if (not args.use_parallel):
        os.environ['CUDA_VISIBLE_DEVICES'] = '0'
        
    # 1. load data
    dataset = MultiModalDataset(args, args.test_annotation, args.test_zip_feats, test_mode=True)
    sampler = SequentialSampler(dataset)
    dataloader = DataLoader(dataset,
                            batch_size=args.test_batch_size,
                            sampler=sampler,
                            drop_last=False,
                            pin_memory=True,
                            num_workers=args.num_workers,
                            prefetch_factor=args.prefetch)

    # 2. load model
    model = ALBEF(args)
    checkpoint = torch.load(args.ckpt_file, map_location='cpu')
    model.load_state_dict(checkpoint['model_state_dict'])
    if torch.cuda.is_available():
        model = torch.nn.parallel.DataParallel(model.cuda())
    model.eval()

    # 3. inference
    prediction_ids = []
    prediction_probs = np.empty((0,200))
    with torch.no_grad():
        for batch in dataloader:
            pred_label_id, pred_prob = model(batch, inference=True)
            prediction_ids.extend(pred_label_id.cpu().numpy())
            prediction_probs = np.vstack((prediction_probs,pred_prob.cpu().numpy()))

    # 4. dump results
    os.makedirs('data/results/'+args.test_output_path, exist_ok=True)
    
    with open(f'data/results/{args.test_output_path}/result_label.csv', 'w+') as f:
        for pred_label_id, ann in zip(prediction_ids, dataset.anns):
            video_id = ann['id']
            category_id = lv2id_to_category_id(pred_label_id)
            f.write(f'{video_id},{category_id}\n')
    
    np.savetxt(f'data/results/{args.test_output_path}/result_prob.csv', prediction_probs, delimiter=",")

if __name__ == '__main__':
    inference()
