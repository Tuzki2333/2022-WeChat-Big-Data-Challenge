...
def swin_tiny(pretrained=None):
    model = SwinTransformerV2(img_size=256, num_classes=0, window_size=8)
    if pretrained is not None:
        checkpoint = torch.load(pretrained, map_location='cpu')['model']
        model.load_state_dict(checkpoint, strict=False)
    return model

def swin_base(pretrained=None):
    model = SwinTransformerV2(img_size=256, embed_dim=128, depths=[2, 2, 18, 2], num_heads=[4, 8, 16, 32], num_classes=0, window_size=16)
    if pretrained is not None:
        checkpoint = torch.load(pretrained, map_location='cpu')['model']
        model.load_state_dict(checkpoint, strict=False)
    return model